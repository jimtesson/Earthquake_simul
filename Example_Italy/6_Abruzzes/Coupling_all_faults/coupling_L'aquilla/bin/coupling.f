...
!      character*6
      character (len=500) name,filename,tmp_name,format_string,
...
c               !!!!!!!!!!!! CFS map  !!!!!!!!!!!!
c               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               if((icfsmap.eq.1).AND.(icfsmapfault(i).eq.1)) then
               write(*,*)'Caluclate CFS map',i,j
               
               !!!!! file name
        			if (i < 10) then
        				if(j < 10) then
            		format_string = "(A1,I1,A1,I1)"
            			else
            		format_string = "(A1,I1,A1,I2)"	
            			endif
        			else
        				if(j < 10) then
            		format_string = "(A1,I2,A1,I1)"
            			else
            		format_string = "(A1,I2,A1,I2)"	
            			endif
        			endif
        			!write(*,*)'blop'
        			write (tmp_name,format_string) "_", i,"_",j
        			
        		! cfs file
        		filplotcfs = trim('../output/CFS/c_')
     .				//trim(name)//trim(tmp_name)
            ...
            elseif (imap.eq.1 .and. j.eq.i) then !.and. ipass.eq.1) then
               write(*,*) ' Calcul de la carte '
 				...
 				!!!!! file name
        			...
        			write (tmp_name,format_string) "_", i
        		! displacement file
        		filslip = trim('../output/SLIP/u_')
     .				//trim(name)//trim(tmp_name)
...
c					 !!!! var1
                     ...
                     !write output
                     !write(iplot,250) 0.001*xm,0.001*ym,-0.001*zmi,var
                     !write(iallvar,260) 0.001*xm,0.001*ym,-0.001*zmi,
!     .                                  var1,var2,var3,var4,var5,
!     .                                  var6,var7,var8,var9,var12
                     write(iplotslip,290)i,0.001*xm,0.001*ym,-0.001*zmi,
...
c					 !!!! var1
     ...
     .                     0.001*xydist, var1,var2,var3,var4
...
c
      if (iecho .ge. 1) then
         do i=1,nfault !WRITE THE MATRIX
...
250   format (11e15.4)
260   format (13e15.4)
270   format (2i10,7e15.4)!to change
280   format (13e15.4)
290   format (1i2.0,8e15.4)! to change
c     
      end
c
c*****************************************************************
